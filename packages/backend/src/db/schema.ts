...
export const changelogEntries = sqliteTable('changelog_entries', {
  ...
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull(),
  // Git-related fields
  gitCommitHash: text('git_commit_hash'), // Link to git commit if auto-generated
  isAutoGenerated: integer('is_auto_generated', { mode: 'boolean' }).notNull().default(false),
});

// Git commits table for tracking processed commits
export const gitCommits = sqliteTable('git_commits', {
  ...
  subject: text('subject').notNull(), // First line of commit message
  body: text('body'), // Rest of commit message
  filesChanged: text('files_changed'), // JSON array of changed files
  insertions: integer('insertions').notNull().default(0),
  deletions: integer('deletions').notNull().default(0),
  // Categorization
  category: text('category', {
    enum: ['feature', 'bugfix', 'improvement', 'breaking', 'security', 'deprecated', 'chore', 'docs', 'test']
  }),
  isBreakingChange: integer('is_breaking_change', { mode: 'boolean' }).notNull().default(false),
  isPublic: integer('is_public', { mode: 'boolean' }).notNull().default(true), // Include in public changelog
  // Processing metadata
  processedAt: integer('processed_at', { mode: 'timestamp' }).notNull(),
  version: text('version'), // Associated version/release
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull(),
}, (table) => ({
  hashIdx: index('git_commits_hash_idx').on(table.hash),
  dateIdx: index('git_commits_date_idx').on(table.date),
  categoryIdx: index('git_commits_category_idx').on(table.category),
  versionIdx: index('git_commits_version_idx').on(table.version),
}));

// Commit filtering rules for excluding commits from public changelogs
export const commitFilters = sqliteTable('commit_filters', {
  id: integer('id').primaryKey(),
  name: text('name').notNull(),
  description: text('description'),
  pattern: text('pattern').notNull(), // Regex pattern to match commit messages
  filterType: text('filter_type', {
    enum: ['exclude', 'include', 'category_override']
  }).notNull().default('exclude'),
  category: text('category', {
    enum: ['feature', 'bugfix', 'improvement', 'breaking', 'security', 'deprecated', 'chore', 'docs', 'test']
  }), // For category_override type
  isActive: integer('is_active', { mode: 'boolean' }).notNull().default(true),
  priority: integer('priority').notNull().default(0), // Higher priority rules are applied first
  createdAt: integer('created_at', { mode: 'timestamp' }).notNull(),
  updatedAt: integer('updated_at', { mode: 'timestamp' }).notNull(),
}, (table) => ({
  priorityIdx: index('commit_filters_priority_idx').on(table.priority),
  activeIdx: index('commit_filters_active_idx').on(table.isActive),
}));
